...
### 1.1 例外：节点按文件名读取的**支撑组件**不改名（2026-09-11 实测沉淀）

**判断标准**：这个文件是不是由 ComfyUI 节点从模型目录里**按文件名枚举并选中**的加载项？是 → **不套用「日期_用途」前缀**。

真实代价（2026-09-11 实际发生、随后回退）：给 4 个 SeedVR2 放大链文件与 1 个 H3 latent upscaler 补上「日期待核_用途_」前缀后，引用它们的
**用户已保存工作流 17 个 + 节点自带示例工作流 4 个**当场失配——节点自带的示例用户无法自行修正。
而这 5 件的作者原始文件名本身已表意清楚（`seedvr2_ema_3b_fp16`、`seedvr2_3b_int8_convrot`、`ema_vae_fp16`、
`minimax_h3_latent_upscaler_3d_fp32`），改名只为补前缀、收益极小。**已全部回退。**

规则：
1. 支撑组件（放大链权重、latent upscaler、独立加载的 VAE／编码器、检测器 ONNX 等）**保留作者原始文件名**；它们的身分与来源由模型卡承载，不靠文件名。
2. 反过来，**LoRA／底模**这类"在提示词与配方里挑、不按文件名被节点枚举"的件，才适用「日期_具体用途_原名」。
3. 任何改名动盘之前，先跑**下游引用扫描**（扫 `ComfyUI\user\` 与 `custom_nodes\` 下的 `*.json`），把"用户工作流 N 个／节点示例 M 个"写进决策依据——**代价数字要摆上台面**，不能只算好处。
4. 发布器的 `request_id` 必须**逐次唯一**：同一 id 用于不同载荷会被拒绝（"同一 request_id 已用于不同载荷；拒绝覆盖"），此时磁盘可能已改名而卡面未更新——必须换新 id 补发布，再复查库存（应回到 `unregistered_files 0`）。
